...
TEXT_FOLDER = "/Users/daivikpurani/Desktop/ACAD/Thesis/code/Backend/scrapeddata"  # Absolute path to text files folder
...
# === PROCESS TEXT FILES ===
def load_and_split_text_files(text_folder):
    print(f"Looking for text files in: {text_folder}")
    documents = []
    text_files = glob.glob(os.path.join(text_folder, "*.txt"))
    print(f"Found {len(text_files)} text files: {text_files}")
    
    if not text_files:
        print("Warning: No text files found in the specified directory!")
        return documents
    ...
    for text_path in text_files:
        ...
        try:
            ...
            print(f"Successfully loaded {text_path}")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    # Load all PDFs
    # pdf_documents = load_and_split_pdfs(PDF_FOLDER)
    # print(f"\nLoaded {len(pdf_documents)} documents from PDFs")
    
    # Load all text files
    text_documents = load_and_split_text_files(TEXT_FOLDER)
    print(f"\nLoaded {len(text_documents)} documents from text files")
    ...
    print(f"All chunks successfully uploaded to Pinecone")
